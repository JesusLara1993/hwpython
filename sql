...
S
ELECT CONCAT(first_name, ' ', last_name) AS "Actor Name" FROM actor;
...
-- 3a. You want to keep a description of each actor. You don't think you will be performing queries on a description, so create a column in the table actor named description and use data type BLOB (Make sure to research the type BLOB, as the difference between it  and VARCHAR are significant).

ALTER TABLE actor
ADD COLUMN description BLOB(100);

-- 3b. Very quickly you realize that entering descriptions for each actor is too much effort.  elete the description column.

ALTER TABLE actor
  DROP COLUMN description;

-- 4a. List the last names of actors, as well as how many actors have that last name.
SELECT last_name, COUNT(*) AS how_many FROM actor
GROUP BY last_name;
...
SELECT REPLACE('first_name', 'GROUCHO', 'HARPO') FROM actor
WHERE last_name= 'WILLIAMS';

-- 4d. Perhaps we were too hasty in changing GROUCHO to HARPO. It turns out that GROUCHO was the correct name after all! In a single query,
-- if the first name of the actor is currently HARPO, change it to GROUCHO.

SELECT REPLACE('first_name', 'HARPO', 'GROUCHO') FROM actor;
...
SELECT staff.first_name, staff.last_name, SUM(payment.amount) AS "Payment Amount"
FROM staff
...
SELECT film.title, COUNT(inventory.film_id) AS 'No. of Copies'
FROM film
...
SELECT store.store_id, SUM(payment.amount) AS "Total Store Revenues" FROM payment
